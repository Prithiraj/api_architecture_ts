...
import { templateDependency_ajv } from '../../schemas/mapper_ajv'

export async function insert_templateDependency(input: any) {
  // procesor
  ...
  let pk_value: string|number = `template_dependency_${dataKey(6)}`;
  const type_of_pk = templateDependency_ajv.id.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO template_dependency (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
