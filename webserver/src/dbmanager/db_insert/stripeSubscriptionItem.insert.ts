...
import { stripeSubscriptionItem_ajv } from '../../schemas/mapper_ajv'

export async function insert_stripeSubscriptionItem(input: any) {
  // procesor
  ...
  let pk_value: string|number = `stripe_subscription_item_${dataKey(6)}`;
  const type_of_pk = stripeSubscriptionItem_ajv.id.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO stripe_subscription_item (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
