...
import { contactReminder_ajv } from '../../schemas/mapper_ajv'

export async function insert_contactReminder(input: any) {
  // procesor
  ...
  let pk_value: string|number = `contact_reminder_${dataKey(6)}`;
  const type_of_pk = contactReminder_ajv.id.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO contact_reminder (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
