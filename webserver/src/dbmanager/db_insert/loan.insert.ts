...
import { loan_ajv } from '../../schemas/mapper_ajv'

export async function insert_loan(input: any) {
  // procesor
  ...
  let pk_value: string|number = `loan_${dataKey(6)}`;
  const type_of_pk = loan_ajv.id.api.type;
