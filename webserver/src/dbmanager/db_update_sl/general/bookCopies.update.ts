...
export async function update_bookCopies(request: any) {
	// processors
  ...
  const update_query = {
    text: `UPDATE public.book_copies SET ${key_value_placeholders} WHERE ${pk}=$${index + 1} ${createdBy} returning ${all_cols_str}`,
    values: values
  }; 
