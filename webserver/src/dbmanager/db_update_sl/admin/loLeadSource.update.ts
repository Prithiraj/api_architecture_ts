...
export async function update_loLeadSource_admin(request: any) {
  // processors
  ...
  const update_query = {
	  text: `UPDATE public.lo_lead_source SET ${key_value_placeholders} WHERE ${pk}=$${index + 1} ${createdBy} returning ${all_cols_str}`,
	  values: values
  }; 
