...
export async function update_bookShelves_admin(request: any) {
  // processors
  ...
  const pk: string[] = [table_cols.shelfId, ]
  ...
  values = values.concat([input.shelfId, ])
  ...
  const update_query = {
	  text: `UPDATE public.book_shelves SET ${key_value_placeholders} WHERE ${pk}=$${index + 1} ${createdBy} returning ${all_cols_str}`,
	  values: values
  }; 
