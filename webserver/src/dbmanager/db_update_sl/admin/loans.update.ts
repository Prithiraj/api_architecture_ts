...
export async function update_loans_admin(request: any) {
  // processors
  ...
  const pk: string[] = [table_cols.loanId, ]
  ...
  values = values.concat([input.loanId, ])
  ...
  const update_query = {
	  text: `UPDATE public.loans SET ${key_value_placeholders} WHERE ${pk}=$${index + 1} ${createdBy} returning ${all_cols_str}`,
	  values: values
  }; 
