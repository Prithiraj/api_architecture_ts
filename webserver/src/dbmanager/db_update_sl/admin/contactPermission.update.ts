...
export async function update_contactPermission_admin(request: any) {
  // processors
  ...
  const update_query = {
	  text: `UPDATE public.contact_permission SET ${key_value_placeholders} WHERE ${pk}=$${index + 1} ${createdBy} returning ${all_cols_str}`,
	  values: values
  }; 
