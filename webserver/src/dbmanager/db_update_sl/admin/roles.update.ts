...
export async function update_roles_admin(request: any) {
  // processors
  ...
  const pk: string[] = [table_cols.roleId, ]
  ...
  values = values.concat([input.roleId, ])
  ...
  const update_query = {
	  text: `UPDATE public.roles SET ${key_value_placeholders} WHERE ${pk}=$${index + 1} ${createdBy} returning ${all_cols_str}`,
	  values: values
  }; 
