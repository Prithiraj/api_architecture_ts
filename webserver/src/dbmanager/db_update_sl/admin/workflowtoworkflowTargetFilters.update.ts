...
export async function update_workflowtoworkflowTargetFilters_admin(request: any) {
  // processors
  ...
  const update_query = {
	  text: `UPDATE public._workflowToworkflow_target_filters SET ${key_value_placeholders} WHERE ${pk}=$${index + 1} ${createdBy} returning ${all_cols_str}`,
	  values: values
  }; 
