...
export async function update_eventCategory_admin(request: any) {
  // processors
  ...
  const table_cols: Record<string, string> = {
    ...
    eventTargetedToTableName: 'event_targeted_to_table_name',
    eventCategoryDescription: 'event_category_description',
  ...
  const update_query = {
	  text: `UPDATE public.event_category SET ${key_value_placeholders} WHERE ${pk}=$${index + 1} ${createdBy} returning ${all_cols_str}`,
	  values: values
  }; 
