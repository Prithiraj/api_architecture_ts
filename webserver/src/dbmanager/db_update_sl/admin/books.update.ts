...
export async function update_books_admin(request: any) {
  // processors
  ...
  const pk: string[] = [table_cols.bookId, ]
  ...
  values = values.concat([input.bookId, ])
  ...
  const update_query = {
	  text: `UPDATE public.books SET ${key_value_placeholders} WHERE ${pk}=$${index + 1} ${createdBy} returning ${all_cols_str}`,
	  values: values
  }; 
