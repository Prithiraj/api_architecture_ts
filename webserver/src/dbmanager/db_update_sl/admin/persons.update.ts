...
export async function update_persons_admin(request: any) {
  // processors
  ...
  const pk: string[] = [table_cols.personid, ]
  ...
  values = values.concat([input.personid, ])
  ...
  const update_query = {
	  text: `UPDATE public.persons SET ${key_value_placeholders} WHERE ${pk}=$${index + 1} ${createdBy} returning ${all_cols_str}`,
	  values: values
  }; 
