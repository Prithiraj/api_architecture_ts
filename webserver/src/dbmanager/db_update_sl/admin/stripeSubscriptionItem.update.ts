...
export async function update_stripeSubscriptionItem_admin(request: any) {
  // processors
  ...
  const update_query = {
	  text: `UPDATE public.stripe_subscription_item SET ${key_value_placeholders} WHERE ${pk}=$${index + 1} ${createdBy} returning ${all_cols_str}`,
	  values: values
  }; 
