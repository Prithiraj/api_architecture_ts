...
export async function update_routing_admin(request: any) {
  // processors
  ...
  const pk: string[] = [table_cols.path, ]
  ...
  values = values.concat([input.path, ])
  ...
  const update_query = {
	  text: `UPDATE public.routing SET ${key_value_placeholders} WHERE ${pk}=$${index + 1} ${createdBy} returning ${all_cols_str}`,
	  values: values
  }; 
