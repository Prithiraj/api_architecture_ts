...
export async function update_contactCustomField_admin(request: any) {
  // processors
  ...
  const update_query = {
	  text: `UPDATE public.contact_custom_field SET ${key_value_placeholders} WHERE ${pk}=$${index + 1} ${createdBy} returning ${all_cols_str}`,
	  values: values
  }; 
