...
export async function update_bridgeContactPrimaryIncome_admin(request: any) {
  // processors
  ...
  const update_query = {
	  text: `UPDATE public.bridge_contact_primary_income SET ${key_value_placeholders} WHERE ${pk}=$${index + 1} ${createdBy} returning ${all_cols_str}`,
	  values: values
  }; 
