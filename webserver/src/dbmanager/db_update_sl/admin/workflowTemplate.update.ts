...
export async function update_workflowTemplate_admin(request: any) {
  // processors
  ...
  const update_query = {
	  text: `UPDATE public.workflow_template SET ${key_value_placeholders} WHERE ${pk}=$${index + 1} ${createdBy} returning ${all_cols_str}`,
	  values: values
  }; 
