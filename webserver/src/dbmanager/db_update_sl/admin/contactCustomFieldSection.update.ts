...
export async function update_contactCustomFieldSection_admin(request: any) {
  // processors
  ...
  const update_query = {
	  text: `UPDATE public.contact_custom_field_section SET ${key_value_placeholders} WHERE ${pk}=$${index + 1} ${createdBy} returning ${all_cols_str}`,
	  values: values
  }; 
