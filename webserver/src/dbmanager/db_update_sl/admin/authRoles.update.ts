...
export async function update_authRoles_admin(request: any) {
  // processors
  ...
  const update_query = {
	  text: `UPDATE public.auth_roles SET ${key_value_placeholders} WHERE ${pk}=$${index + 1} ${createdBy} returning ${all_cols_str}`,
	  values: values
  }; 
