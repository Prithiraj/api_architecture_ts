...
export async function update_partner_admin(request: any) {
  // processors
  ...
  const update_query = {
	  text: `UPDATE public.partner SET ${key_value_placeholders} WHERE ${pk_str} ${createdBy} returning ${all_cols_str}`,
	  values: values
  }; 
