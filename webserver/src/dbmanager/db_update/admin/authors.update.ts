...
export async function update_authors_admin(request: any) {
  // processors
  ...
  const pk: string[] = [table_cols.authorId, ]
  ...
  values = values.concat([input.authorId, ])
  ...
  const update_query = {
	  text: `UPDATE public.authors SET ${key_value_placeholders} WHERE ${pk_str} ${createdBy} returning ${all_cols_str}`,
	  values: values
  }; 
