...
export async function update_members_admin(request: any) {
  // processors
  ...
  const pk: string[] = [table_cols.memberId, ]
  ...
  values = values.concat([input.memberId, ])
  ...
  const update_query = {
	  text: `UPDATE public.members SET ${key_value_placeholders} WHERE ${pk_str} ${createdBy} returning ${all_cols_str}`,
	  values: values
  }; 
