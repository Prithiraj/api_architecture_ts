...
export async function update_bridgeContactLoan_admin(request: any) {
  // processors
  ...
  values = values.concat([input.contactId, input.loanId, ])
  ...
  const update_query = {
	  text: `UPDATE public.bridge_contact_loan SET ${key_value_placeholders} WHERE ${pk_str} ${createdBy} returning ${all_cols_str}`,
	  values: values
  }; 
