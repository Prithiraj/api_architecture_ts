...
export async function update_eventsV2_admin(request: any) {
  // processors
  ...
  const update_query = {
	  text: `UPDATE public.events_v2 SET ${key_value_placeholders} WHERE ${pk_str} ${createdBy} returning ${all_cols_str}`,
	  values: values
  }; 
