...
export async function update_loSubscriptionDailyStats_admin(request: any) {
  // processors
  ...
  const pk: string[] = [table_cols.loSubscriptionId, table_cols.statsDate, ]
  ...
  values = values.concat([input.loSubscriptionId, input.statsDate, ])
  ...
  const update_query = {
	  text: `UPDATE public.lo_subscription_daily_stats SET ${key_value_placeholders} WHERE ${pk_str} ${createdBy} returning ${all_cols_str}`,
	  values: values
  }; 
