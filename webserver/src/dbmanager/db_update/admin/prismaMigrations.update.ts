...
export async function update_prismaMigrations_admin(request: any) {
  // processors
  ...
  const update_query = {
	  text: `UPDATE public._prisma_migrations SET ${key_value_placeholders} WHERE ${pk_str} ${createdBy} returning ${all_cols_str}`,
	  values: values
  }; 
