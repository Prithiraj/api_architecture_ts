...
export async function update_bridgeSectionLifecycle_admin(request: any) {
  // processors
  ...
  const update_query = {
	  text: `UPDATE public.bridge_section_lifecycle SET ${key_value_placeholders} WHERE ${pk_str} ${createdBy} returning ${all_cols_str}`,
	  values: values
  }; 
