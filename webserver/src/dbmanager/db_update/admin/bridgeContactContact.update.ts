...
export async function update_bridgeContactContact_admin(request: any) {
  ...
  const pk: string[] = [table_cols.contactId, table_cols.assignedTo, ]
  ...
  values = values.concat([input.contactId, input.assignedTo, ])
  ...
  const update_query = {
	  text: `UPDATE public.bridge_contact_contact SET ${key_value_placeholders} WHERE ${pk_str} ${createdBy} returning ${all_cols_str}`,
	  values: values
  }; 
