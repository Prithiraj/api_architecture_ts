...
import { bridgeSectionLifecycle_ajv } from '../../schemas/mapper_ajv'

export async function insert_bridgeSectionLifecycle(request: any) {
  // procesor
  ...
  let pk_value: string|number = `bridge_section_lifecycle_${dataKey(6)}`;
  const type_of_pk = bridgeSectionLifecycle_ajv.id.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO bridge_section_lifecycle (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
