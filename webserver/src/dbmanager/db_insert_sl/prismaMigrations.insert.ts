...
import { prismaMigrations_ajv } from '../../schemas/mapper_ajv'

export async function insert_prismaMigrations(request: any) {
  // procesor
  ...
  let pk_value: string|number = `_prisma_migrations_${dataKey(6)}`;
  const type_of_pk = prismaMigrations_ajv.id.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO _prisma_migrations (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
