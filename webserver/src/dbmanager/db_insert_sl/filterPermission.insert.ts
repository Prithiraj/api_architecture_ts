...
import { filterPermission_ajv } from '../../schemas/mapper_ajv'
...
export async function insert_filterPermission(request: any) {
  ...
  let pk_value: string|number = `filter_permission_${dataKey(6)}`;
  const type_of_pk = filterPermission_ajv.id.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO filter_permission (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
