...
import { organization_ajv } from '../../schemas/mapper_ajv'

export async function insert_organization(request: any) {
  // procesor
  ...
  let pk_value: string|number = `organization_${dataKey(6)}`;
  const type_of_pk = organization_ajv.id.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO organization (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
