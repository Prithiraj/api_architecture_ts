...
import { bridgeContactTag_ajv } from '../../schemas/mapper_ajv'

export async function insert_bridgeContactTag(request: any) {
  // procesor
  ...
  let pk_value: string|number = `bridge_contact_tag_${dataKey(6)}`;
  const type_of_pk = bridgeContactTag_ajv.contactId.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO bridge_contact_tag (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
