...
import { routing_ajv } from '../../schemas/mapper_ajv'

export async function insert_routing(request: any) {
  // procesor
  ...
  let pk_value: string|number = `routing_${dataKey(6)}`;
  const type_of_pk = routing_ajv.path.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO routing (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
