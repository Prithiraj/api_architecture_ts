...
import { loSubscription_ajv } from '../../schemas/mapper_ajv'

export async function insert_loSubscription(request: any) {
  // procesor
  ...
  let pk_value: string|number = `lo_subscription_${dataKey(6)}`;
  const type_of_pk = loSubscription_ajv.id.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO lo_subscription (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
