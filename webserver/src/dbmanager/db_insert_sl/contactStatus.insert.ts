...
import { contactStatus_ajv } from '../../schemas/mapper_ajv'

export async function insert_contactStatus(request: any) {
  // procesor
  ...
  let pk_value: string|number = `contact_status_${dataKey(6)}`;
  const type_of_pk = contactStatus_ajv.id.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO contact_status (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
