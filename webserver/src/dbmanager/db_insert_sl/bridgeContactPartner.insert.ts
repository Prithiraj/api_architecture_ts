...
import { bridgeContactPartner_ajv } from '../../schemas/mapper_ajv'

export async function insert_bridgeContactPartner(request: any) {
  // procesor
  ...
  let pk_value: string|number = `bridge_contact_partner_${dataKey(6)}`;
  const type_of_pk = bridgeContactPartner_ajv.id.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO bridge_contact_partner (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
