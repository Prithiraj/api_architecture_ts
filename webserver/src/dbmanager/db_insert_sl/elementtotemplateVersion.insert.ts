...
export async function insert_elementtotemplateVersion(request: any) {
  // procesor
  ...
  let pk_value: string|number = `_elementTotemplate_version_${dataKey(6)}`;
  const type_of_pk = elementtotemplateVersion_ajv.id.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO _elementTotemplate_version (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
