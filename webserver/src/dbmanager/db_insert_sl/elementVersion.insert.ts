...
import { elementVersion_ajv } from '../../schemas/mapper_ajv'

export async function insert_elementVersion(request: any) {
  // procesor
  ...
  let pk_value: string|number = `element_version_${dataKey(6)}`;
  const type_of_pk = elementVersion_ajv.id.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO element_version (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
