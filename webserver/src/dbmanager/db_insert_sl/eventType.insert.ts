...
export async function insert_eventType(request: any) {
  ...
  let pk_value: string|number = `event_type_${dataKey(6)}`;
  const type_of_pk = eventType_ajv.id.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO event_type (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
