...
import { workflowTemplate_ajv } from '../../schemas/mapper_ajv'

export async function insert_workflowTemplate(request: any) {
  // procesor
  ...
  let pk_value: string|number = `workflow_template_${dataKey(6)}`;
  const type_of_pk = workflowTemplate_ajv.id.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO workflow_template (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
