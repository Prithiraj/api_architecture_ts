...
import { authUserRoles_ajv } from '../../schemas/mapper_ajv'

export async function insert_authUserRoles(request: any) {
  // procesor
  ...
  let pk_value: string|number = `auth_user_roles_${dataKey(6)}`;
  const type_of_pk = authUserRoles_ajv.id.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO auth_user_roles (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
