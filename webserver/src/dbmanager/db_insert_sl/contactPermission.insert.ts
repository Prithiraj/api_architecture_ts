...
import { contactPermission_ajv } from '../../schemas/mapper_ajv'
...
export async function insert_contactPermission(request: any) {
  ...
  let pk_value: string|number = `contact_permission_${dataKey(6)}`;
  const type_of_pk = contactPermission_ajv.id.api.type;
