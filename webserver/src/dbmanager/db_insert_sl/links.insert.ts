...
import { links_ajv } from '../../schemas/mapper_ajv'

export async function insert_links(request: any) {
  // procesor
  ...
  let pk_value: string|number = `links_${dataKey(6)}`;
  const type_of_pk = links_ajv.id.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO links (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
