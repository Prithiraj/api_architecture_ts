...
import { bookCopies_ajv } from '../../schemas/mapper_ajv'

export async function insert_bookCopies(request: any) {
  // procesor
  ...
  let pk_value: string|number = `book_copies_${dataKey(6)}`;
  const type_of_pk = bookCopies_ajv.copyNumber.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO book_copies (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
