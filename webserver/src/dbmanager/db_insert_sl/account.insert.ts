...
import { account_ajv } from '../../schemas/mapper_ajv'

export async function insert_account(request: any) {
  // procesor
  ...
  let pk_value: string|number = `account_${dataKey(6)}`;
  const type_of_pk = account_ajv.id.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO account (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
