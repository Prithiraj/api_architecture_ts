...
import { pageVersion_ajv } from '../../schemas/mapper_ajv'

export async function insert_pageVersion(request: any) {
  // procesor
  ...
  let pk_value: string|number = `page_version_${dataKey(6)}`;
  const type_of_pk = pageVersion_ajv.id.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO page_version (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
