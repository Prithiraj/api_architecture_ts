...
import { contactFilter_ajv } from '../../schemas/mapper_ajv'

export async function insert_contactFilter(request: any) {
  // procesor
  ...
  let pk_value: string|number = `contact_filter_${dataKey(6)}`;
  const type_of_pk = contactFilter_ajv.id.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO contact_filter (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
