...
export async function insert_bridgeContactUser(request: any) {
  // procesor
  ...
  let pk_value: string|number = `bridge_contact_user_${dataKey(6)}`;
  const type_of_pk = bridgeContactUser_ajv.contactId.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO bridge_contact_user (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
