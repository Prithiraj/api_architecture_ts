...
import { bridgeContactPrimaryIncome_ajv } from '../../schemas/mapper_ajv'
...
export async function insert_bridgeContactPrimaryIncome(request: any) {
  ...
  let pk_value: string|number = `bridge_contact_primary_income_${dataKey(6)}`;
  const type_of_pk = bridgeContactPrimaryIncome_ajv.id.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO bridge_contact_primary_income (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
