...
import { contactTag_ajv } from '../../schemas/mapper_ajv'

export async function insert_contactTag(request: any) {
  // procesor
  ...
  let pk_value: string|number = `contact_tag_${dataKey(6)}`;
  const type_of_pk = contactTag_ajv.id.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO contact_tag (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
