...
import { template_ajv } from '../../schemas/mapper_ajv'

export async function insert_template(request: any) {
  // procesor
  ...
  let pk_value: string|number = `template_${dataKey(6)}`;
  const type_of_pk = template_ajv.id.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO template (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
