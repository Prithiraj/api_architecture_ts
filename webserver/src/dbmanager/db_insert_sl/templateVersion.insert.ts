...
import { templateVersion_ajv } from '../../schemas/mapper_ajv'

export async function insert_templateVersion(request: any) {
  // procesor
  ...
  let pk_value: string|number = `template_version_${dataKey(6)}`;
  const type_of_pk = templateVersion_ajv.id.api.type;
  ...
  const insert_query = {
	  text: `INSERT INTO template_version (${cols_str}) values (${placeholders}) returning ${all_cols_str}`,
	  values: values
  };
