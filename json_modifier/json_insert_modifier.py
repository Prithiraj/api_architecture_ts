class JsonInsertModifier:
    ...
    def preloadColumnInfoSchemaList(self, data, schema_level_rules):
        ...
        # As composite keys aren't autogenerated, hence it shouldn't be excluded from the input.
        len_pks = len([pk for pk in data["columnInfoList"] if pk["primarykey"]=='t'])
        for field in data["columnInfoList"]:
            if bool(field["foreignkey"]):
                fk_list.append(field["foreignkey"])
            
            if (schema_level_rules["insert"]["exclude"]["primaryKey"]==True and field["primarykey"]=='t') and len_pks == 1:
